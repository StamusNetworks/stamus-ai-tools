...
- Always try to set the protocol layer in the signature as the second parameter of the rule (e.g., http, tls, smb).
- Port specification is useless when the second parameter of the rule is set to the application layer (e.g., http, tls, smb) as the port is not relevant for application layer signatures. In that case, use `any` for the port specification.
