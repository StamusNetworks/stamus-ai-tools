...
- When a signature is likely to trigger a lot because of repeated actions (e.g., a policy violation signature for a specific domain), consider adding a threshold to limit the number of alerts generated by that signature. Threshold has to be setup to avoid missing anny asset exhibiting the behavior, for example with `threshold:type limit, track by_src, count 1, seconds 60;` to limit to 5 alerts per minute per source IP.
